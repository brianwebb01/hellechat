...
(() => {
/*!*******************************************!*\
  !*** ./resources/js/contactManagement.js ***!
  \*******************************************/
__webpack_require__.r(__webpack_exports__);
...
window.manageContacts = function () {
  return {
    ...
    searchContacts: function searchContacts() {
      ...
      var result = fuse.search(this.searchTerm);
      this.records = result.map(function (i) {
        return i.item;
      });
      this.regroupRecords();
    },
    regroupRecords: function regroupRecords() {
    ...
    setPhoneNumbers: function setPhoneNumbers(numbers) {
      var _this = this;

      if (typeof numbers == 'string') numbers = JSON.parse(numbers);
      this.phoneNumbers = Object.keys(numbers).map(function (k) {
        return {
          'type': k,
          'number': numbers[k],
          'id': _this.randomId()
    ...
    groupByLastFirst: function groupByLastFirst(array) {
      var grouped = array.reduce(function (acc, obj) {
        var letter = (obj.last_name ? obj.last_name.charAt(0) : obj.first_name ? obj.first_name.charAt(0) : obj.company.charAt(0)).toUpperCase();
      ...
      Object.keys(grouped).forEach(function (key) {
        var sorted = grouped[key];
        sorted.sort(function (a, b) {
          var asort = (a.last_name || a.company || a.first_name).toLowerCase();
          var bsort = (b.last_name || b.company || b.first_name).toLowerCase();
    ...
    getAvatarLetters: function getAvatarLetters(contact) {
      var letters = [];
    ...
    deletePhone: function deletePhone(index) {
      var item = this.phoneNumbers[index];
      this.phoneNumbers = this.phoneNumbers.filter(function (i) {
        return i.id != item.id;
      });
    },
    beforeSave: function beforeSave() {
      ...
      this.phoneNumbers.forEach(function (item) {
        result["".concat(item.type)] = item.number;
      });
